...
app = Flask(__name__, static_url_path="/D:/workspace/DOAN/output", static_folder="/D:/workspace/DOAN/output")
CORS(app)

app.config["UPLOAD_FOLDER"] = "D:\\workspace\\DOAN\\uploads"
app.config["COMPRESSED_FOLDER"] = "D:\\workspace\\DOAN\\compressed"
app.config["OUTPUT_FOLDER"] = "D:\\workspace\\DOAN\\output"
    ...
    def in_order(self):
        if self is not None:
            if self.c[0] is not None:
                self.c[0].in_order()
            print(self.freq, self.prob, self.word)
            print("\n")
...
def process_image(file_path, file_name):
    ...
    compressed_path = os.path.join(
        app.config["COMPRESSED_FOLDER"], f"{file_name}.bin"
    )
...
def upload_image():
    ...
    if file:
        ...
        executor.submit(process_image, file_path, file.filename)
        

        return jsonify({"message": "Processing initiated", "filename": file.filename, "filesize": file_size})
...
def status(filename):
    ...
    if os.path.exists(output_path):
        ...
        return jsonify({"status": "done", "url": filename, "filesize": filesize})
    else:
